...
data1$FrDeltaMass <- data1$FrDeltaMass * 1000000 / data1$Precursor_Mass;
...
plot_medium <- function(dataF, name1, name2)
{
	...
	z2 = c(NA)
	for (i in 1:bins){
	z2[i] <- median(as.numeric(dataF[y==i,name1]))
	}
	for (i in 1:1) {
...
op = par(mfrow=c(2,1))
plot(as.numeric(data1$X.fraction), data1$Delta_Mass, xlab = "fractions", ylab = "Delta_Mass",  main = "Delta Mass Per Fraction")
axis(1,xaxp=c(1,100,99))
abline(h=0, col=2)
abline(h=1, col=2)
abline(h=-1, col=2)
data_s1 = data1[data1$Delta_Mass <= 0.5 & data1$Delta_Mass >=-0.5,]
plot_medium(data_s1, "X.fraction", "Delta_Mass")
data_s2 = data1[data1$Delta_Mass <= 1.5 & data1$Delta_Mass >=0.5,]
plot_medium(data_s2, "X.fraction", "Delta_Mass")
data_s3 = data1[data1$Delta_Mass < -0.5 & data1$Delta_Mass >=-1.5,]
plot_medium(data_s3, "X.fraction", "Delta_Mass")
####plot fractional delta mass
plot(as.numeric(data1$X.fraction), data1$FrDeltaMass, xlab = "fractions", ylab = "Fractional_Delta_Mass",  main = "Fractional Delta Mass Per Fraction (ppm)")
axis(1,xaxp=c(1,100,99))
abline(h=0, col=2)
plot_medium(data1, "X.fraction", "FrDeltaMass")
par(op)
dev.off()

png(file = delta_mass_boxplots_file, height = 800, width = 1000, pointsize=12, bg = "white")
boxplot(data1$FrDeltaMass~as.factor(data1$X.fraction), xlab = "fractions", ylab = "Fractional_Delta_Mass",  main = "Fractional Delta Mass Per Fraction (ppm)")
abline(h=0, col = 2)
dev.off()
...
plot(dataS$total_peptide~dataS$number, xlab = "fractions", ylab = "counts", ylim=c(0, max(dataS$total_peptide)), col="white")
...
{plot(dataS$protein_groups~dataS$number, xlab = "fractions", ylim=c(0, max(dataS$protein_groups)),ylab = "counts",col="white")}
if(max(dataS$protein_groups) <= max(dataS$gene_symbols))
{plot(dataS$gene_symbol~dataS$number, xlab = "fractions", ylab = "counts", ylim=c(0, max(dataS$gene_symbol)),col="white")}
...
dataA2 <- dataA1[dataA1$Light_Area != 0,]
dataA <- dataA1[dataA2$Light_Area != 0,]
dataA$Ratio = replace(dataA$Decimal_Ratio, dataA$Decimal_Ratio <= 0, 0.001)
...
{
	if(nrow(dataF) > 0){
	plot(dataF[,name1],dataF[,name2], xlab = name1, ylab = name2, main = name3)}
...
{
	file_name = paste("quantified_peps",i, sep="")
	...
	{
		frxn = (i-1)*4+j
		dataF <- dataA[dataA$X.fraction == frxn,]
...
file_name = paste("quantified_peps",num_slides+1, sep="")
file_name = paste(file_name,".png", sep="")
png(file = file_name, height = 600, width = 800, pointsize=12, bg = "white")
op = par(mfrow=c(2,2))
...
plot_quantile2 <- function(dataF, name1, name2)
{
	...
	z2 = c(NA)
	for (i in 1:bins){
	z2[i] = median(dataF[y==i,name1])
	}
	plot(dataF[,name1],dataF[,name2], xlab = name1, ylab = name2)
	for (i in 1:5) {
	lines(z2,z[,i], col=2)
	}
	abline(h=0)
...
write_quantile <- function(dataF, name1, name2, File)  #name1=x value, name2=y value
{
	...
	zz2 <- c(NA)
	for (i in 1:bins){
	zz2[i] <- median(dataF[yy==i,name1])
	}
	zz2[bins+1] <- median(dataF[,name1])
	...
	medium <- round(2^zz[,5],2)
	dataW1 <-data.frame(catagory, frequency, mid_value, medium, percent25, percent50, percent75, percent90)
...
write_quantile2 <- function(dataF, name1, name2, File)  #name1=x value, name2=y value
{
	...
	percent25 <- paste(round(2^zz[,4],2), round(2^zz[,6],2), sep = " - ")
	medium <- round(2^zz[,5],2)
	dataW1 <-data.frame(catagory, frequency, medium, percent25, percent50, percent75, percent90)
	write.csv(dataW1, File, row.names = FALSE)
}
write_quantile(dataA, "Precursor_Mass", "logL2H", "quantile_summary_mass.csv")
write_quantile(dataA, "logLH", "logL2H", "quantile_summary_intensity.csv")
write_quantile2(dataA, "num_labels", "logL2H","quantile_summary_cys.csv")
...
z4 = c(NA)
for (i in 1:bins1){
z4[i] = median(dataP[y==i,]$Ratio_Peps)
}
z4[bins1+1] <- median(dataP$Ratio_Peps)
