...
public class AmtExtraInfoDef extends FeatureExtraInformationDef
{
    ...
    public static void setFeatureSetRunsMatched(FeatureSet featureSet,
                                                AmtRunEntry[] runsMatched)
    {
        if (runsMatched == null)
            return;
        List<String> runsMatchedList = new ArrayList<String>(runsMatched.length);
        for (AmtRunEntry run : runsMatched)
            runsMatchedList.add(run.getPepXmlFilename());
        setFeatureSetRunsMatched(featureSet, runsMatchedList);
    }
