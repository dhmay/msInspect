...
public class ExtractMaxima2D
	...
	static final double RESAMPLE_INTERVAL = 1.0 / RESAMPLE_REQUENCY;
	...
	public static Spectrum.Peak[] analyze(Scan[] scans, FloatRange rangePeaks, Smooth2D smooth2d, float minPeak)
		...
		for (int i = 0; i < scans.length; i++)
			spectra[i + 1] = Spectrum.Resample(scans[i].getSpectrum(), range, RESAMPLE_REQUENCY);
		spectra[0] = spectra[1];
		spectra[spectra.length - 1] = spectra[spectra.length - 2];

		Spectrum.Peak[] peaks = analyze(spectra, range.min, RESAMPLE_INTERVAL, smooth2d, minPeak);
