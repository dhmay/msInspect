...
*/
    ...
    */
    ...
    public static class FeatureBase extends Peak implements Cloneable
    {
...
/*
        ...
        // UNDONE: clean up constructors, we can't need all of these
		public FeatureBase(FeatureBase f)
            {
            this.time = f.time;
            this.scan = f.scan;
            this.mz = f.mz;
            this.intensity = f.intensity;
            this.charge = f.charge;
            this.kl = f.kl;
            this.mass = f.mass;
            this.background = f.background;
            this.setMedian(f.getMedian());
            this.setPeaks(f.getPeaks());
            this.setDescription(f.getDescription());
            this.chargeStates = f.chargeStates;
            }


		public FeatureBase(int scan, float mz, int charge)
			{
			this.scan = scan;
			this.mz = mz;
			this.charge = charge;
		...
		public FeatureBase(int scan, float mz, float intensity, int charge, float kl)
			{
			this.scan = scan;
			this.mz = mz;
			this.intensity = intensity;
		...
		protected void computeMass()
			{
			float m = (mz - HYDROGEN_ION_MASS) * charge;
			m = Math.max(0.0F, m);
			mass = m;
			}


		public FeatureBase(Peak p)
			{
			this(p.scan, p.mz, p.intensity, 0, 0);
			median = p.median;
			background = p.background;
			}


		public static String FEATURE_HEADER = "scan\ttime\tmz\tmass\tintensity\tcharge\tchargeStates\tkl\tbackground\tmedian\tpeaks\tdescription";
		...
		public String toString()
			...
			return scan + "\t" + time + "\t" + mz + "\t" + mass + "\t" + intensity + "\t" + charge + "\t" + chargeStates + "\t" +
			        kl + "\t" + background + "\t" + getMedian() + "\t" + getPeaks() + (null == txt ? "" : "\t\"" + txt + "\"");
			}


		public boolean equals(Object o)
			{
			if (null == o || !(o instanceof FeatureBase))
				return false;

			FeatureBase f = (FeatureBase) o;
			return f.scan == scan && f.mz == mz && f.intensity == intensity && f.charge == charge &&
			        f.kl == kl;
			}
		...
		public static class IntensityDescComparator implements Comparator
			{
			public int compare(Object o1, Object o2)
				{
				return _compareDesc(((FeatureBase) o1).intensity, ((FeatureBase) o2).intensity);
				}
			}

		public static class MassAscComparator implements Comparator
			{
			public int compare(Object o1, Object o2)
				{
				if (((FeatureBase) o1).mass == ((FeatureBase) o2).mass)
					return 0;
				return _compareAsc(((FeatureBase) o1).mass, ((FeatureBase) o2).mass);
				}
			}

		public static class MzDistAscComparator implements Comparator
			{
			public int compare(Object o1, Object o2)
				{
				if (((FeatureBase) o1).mz == ((FeatureBase) o2).mz)
					return _compareAsc(((FeatureBase) o1).kl, ((FeatureBase) o2).kl);
				return _compareAsc(((FeatureBase) o1).mz, ((FeatureBase) o2).mz);
				}
			}

		public static class MzAscComparator implements Comparator
			{
			public int compare(Object o1, Object o2)
				{
				if (((FeatureBase) o1).mz == ((FeatureBase) o2).mz)
					return _compareAsc(((FeatureBase) o1).kl, ((FeatureBase) o2).kl);
				return ((FeatureBase) o1).charge - ((FeatureBase) o2).charge;
				}
			}

		public static class MzScanAscComparator implements Comparator
			{
			public int compare(Object o1, Object o2)
				{
				float mzDiff = ((FeatureBase) o1).mz - ((FeatureBase) o2).mz;

				if (0 == mzDiff)
					return _compareAsc(((FeatureBase) o1).scan, ((FeatureBase) o2).scan);

				return mzDiff > 0 ? 1 : -1;
				}
			}

        public static class ScanAscComparator implements Comparator
            {
            public int compare(Object o1, Object o2)
                {
                return _compareAsc(((FeatureBase) o1).scan, ((FeatureBase) o2).scan);
...
*/
    ...
    /*
      * assumes input has been preprocesses
      *   lightly smoothed
      *   removed background
      *
